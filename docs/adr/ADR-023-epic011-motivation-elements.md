...
- `MotivationElement(Element)` already exists in `src/pyarchi/metamodel/elements.py` (ADR-016).
...
- `Layer.MOTIVATION` and `Aspect.MOTIVATION` already exist in `src/pyarchi/enums.py` (ADR-011, ADR-012).
...
### 1. Module Placement: `src/pyarchi/metamodel/motivation.py`

All EPIC-011 concrete classes are defined in a new module `src/pyarchi/metamodel/motivation.py`. This continues the per-layer module pattern established in ADR-018 Decision 1.
...
Since our classes live in `pyarchi.metamodel.motivation`, there is no conflict. Fully-qualified imports (`from pyarchi.metamodel.motivation import Requirement`) are unambiguous. No renaming or prefixing (e.g., `MotivationRequirement`) is applied -- the spec uses these names directly, and prefixing would violate the naming convention established across all prior layer epics.
...
`Layer.MOTIVATION` (ADR-011) and `Aspect.MOTIVATION` (ADR-012) already exist in `src/pyarchi/enums.py`. EPIC-011 introduces no new enum members or enum classes.

### 9. `__init__.py` Exports Deferred

Exports of Motivation layer types to `src/pyarchi/__init__.py` are deferred to EPIC-014 (public API surface epic), consistent with ADR-016 Decision 7, ADR-018 Decision 8, ADR-019 Decision 11, ADR-020 Decision 11, ADR-021 Decision 13, and ADR-022 Decision 7. The types are importable via `from pyarchi.metamodel.motivation import Stakeholder` immediately.
...
- `Requirement` and `Constraint` are common English words. While no conflict exists at the module level, consumers using star imports (`from pyarchi.metamodel.motivation import *`) alongside `from packaging.requirements import *` would experience shadowing. Star imports are already discouraged by PEP 8.
