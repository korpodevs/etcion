...
| 4 | **API location** | (a) `Model.diff()` method, (b) Standalone function in `comparison.py` | **(b) `src/pyarchi/comparison.py`** | Keeps `Model` focused on containment. Diff is a cross-model operation, not owned by either operand. |
...
| 11 | **Exports** | -- | `ModelDiff`, `diff_models`, `ConceptChange`, `FieldChange` added to `pyarchi.__init__` | Top-level public API. |
