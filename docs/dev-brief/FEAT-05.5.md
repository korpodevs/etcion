# Technical Brief: FEAT-05.5 Dependency Relationships -- Influence
...
Define `Influence(DependencyRelationship)` with `sign: InfluenceSign | None = None` and `strength: str | None = None`. `InfluenceSign` enum already exists in `enums.py` (ratified ADR-017 ss2).
...
| `InfluenceSign` in `enums.py` | Done (ratified ADR-017 ss2) |

## Stories -> Acceptance

| Story | Testable Now? | Acceptance |
|---|---|---|
| 05.5.1 | Tests only | `InfluenceSign` exists with `STRONG_POSITIVE` ("++"), `POSITIVE` ("+"), `NEUTRAL` ("0"), `NEGATIVE` ("-"), `STRONG_NEGATIVE` ("--") |
| 05.5.2 | Yes | `Influence` instantiates; `sign` and `strength` default to `None` |
| 05.5.3 | Yes | Tests confirm defaults |
...
## Test File: `test/test_feat055_influence.py`
...
ruff check src/pyarchi/metamodel/relationships.py test/test_feat055_influence.py
ruff format --check src/pyarchi/metamodel/relationships.py test/test_feat055_influence.py
mypy src/pyarchi/metamodel/relationships.py test/test_feat055_influence.py
pytest test/test_feat055_influence.py -v
