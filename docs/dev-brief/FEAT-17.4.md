# Technical Brief: FEAT-17.4 -- Concern Class

**Status:** Ready for TDD
**ADR:** `docs/adr/ADR-029-epic017-viewpoint-mechanism.md`
**Spec:** ArchiMate 3.2, Section 13.1
...
| Artifact | Location |
|---|---|
| `Concern` | `src/pyarchi/metamodel/viewpoints.py` |
...
NOT frozen -- `stakeholders` and `viewpoints` are mutable lists. NOT a `Concept` subclass.

## Fields

| Field | Type | Required | Default | Mutable |
|---|---|---|---|---|
| `description` | `str` | Yes | -- | Yes |
| `stakeholders` | `list[Stakeholder]` | No | `[]` | Yes |
| `viewpoints` | `list[Viewpoint]` | No | `[]` | Yes |

## Navigation Chain

`Stakeholder` -> `Concern` -> `Viewpoint` -> `View`

Navigation from `Stakeholder` to its concerns is the caller's responsibility (filter a concern collection). `Concern` holds forward references to both `Stakeholder` instances and `Viewpoint` instances.

## Ordering Note

Because `Viewpoint.concerns` references `Concern` and `Concern.viewpoints` references `Viewpoint`, use forward references (`from __future__ import annotations` or string literals) and call `model_rebuild()` as needed to resolve the circular type reference.

## Test File: `test/test_feat174_concern.py`
...
class TestStakeholderConcernViewpointNavigation:
    ...
    def test_reverse_navigation_via_comprehension(
        ...
        stakeholder_concerns = [
            c for c in all_concerns if stakeholder in c.stakeholders
        ]
