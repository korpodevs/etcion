...
A naive flat layout -- placing all of these modules directly under `src/etcion/` -- would produce 100+ files in a single directory, making navigation impractical for both humans and tooling. Conversely, over-segmenting the package by ArchiMate layer (e.g., `src/etcion/business/`, `src/etcion/application/`) would create circular import problems because ArchiMate relationships routinely cross layer boundaries.
...
2. **`src/etcion/__init__.py` is the sole public API surface.** Internal sub-packages are implementation details; they must support clean re-exports through the top-level `__init__.py` without forcing consumers to know the internal structure.

## Decision

### STORY-00.2.1 -- `src/etcion/metamodel/` Sub-package

All metamodel classes -- the abstract base classes, mixins, concrete element types, concrete relationship types, and the Model container -- will live under `src/etcion/metamodel/`.

The initial internal structure of this sub-package is:

```
src/etcion/metamodel/
...
src/etcion/metamodel/relationships/
...
The `metamodel/__init__.py` will re-export the types that downstream sub-packages (validation, derivation) need, so that internal imports read as `from etcion.metamodel import Element, Relationship` rather than reaching into specific files.

### STORY-00.2.2 -- `src/etcion/enums.py`

All enumeration types will live in a single flat module at `src/etcion/enums.py`. The enums defined here are:
...
- Enums are imported by nearly every other module in the library (metamodel, validation, derivation). A single import path (`from etcion.enums import Layer, Aspect`) is simpler and more discoverable than scattering enums across multiple files.
...
### STORY-00.2.3 -- `src/etcion/validation/` Sub-package

Validation logic will live in a dedicated sub-package at `src/etcion/validation/`. This sub-package is responsible for:
...
Exception types referenced by validation (e.g., `ValidationError`) are defined in `src/etcion/exceptions.py` (see STORY-00.2.5), not inside the validation sub-package. The validation sub-package imports from `exceptions`, not the other way around.

### STORY-00.2.4 -- `src/etcion/derivation/` Sub-package

The derivation engine will live in a dedicated sub-package at `src/etcion/derivation/`. This sub-package is responsible for:
...
### STORY-00.2.5 -- `src/etcion/exceptions.py`

All custom exception types will live in a single flat module at `src/etcion/exceptions.py`. The initial exception classes are:

- **`PyArchiError(Exception)`**: Base exception class for all library errors. Allows consumers to catch all etcion errors with a single `except PyArchiError`.
- **`ValidationError(PyArchiError)`**: Raised when a metamodel constraint is violated (invalid relationship source/target, missing required fields, illegal `is_nested` flag, etc.). This is a etcion-specific type, distinct from `pydantic.ValidationError`, to give the library control over error formatting and to avoid coupling consumer error handling to Pydantic internals.
...
- Consumers who want to catch library exceptions need a single, predictable import path: `from etcion.exceptions import ValidationError`.
...
### Flat Layout -- Everything Directly in `src/etcion/`
...
src/etcion/strategy/
src/etcion/business/
src/etcion/application/
src/etcion/technology/
src/etcion/physical/
src/etcion/motivation/
src/etcion/implementation_migration/
```
...
- **Single import path for consumers**: All public types are re-exported through `src/etcion/__init__.py`. Consumers write `from etcion import BusinessActor, Serving, ValidationError` without needing to know the internal package structure.
...
- **Indirection cost for contributors**: A new contributor looking for the `Composition` class must navigate from `src/etcion/` to `metamodel/` to `relationships/` to `structural.py`. This three-level nesting is deeper than a flat layout, but is mitigated by IDE "go to definition" and the predictable naming convention.
...
|---|---|
| STORY-00.2.1 | `src/etcion/metamodel/` sub-package with `concepts.py`, `mixins.py`, `model.py`, `notation.py`, and planned `elements/` and `relationships/` sub-packages |
| STORY-00.2.2 | `src/etcion/enums.py` as a single flat module containing `Layer`, `Aspect`, `RelationshipCategory`, `AccessMode`, `InfluenceSign`, `AssociationDirection`, `JunctionType` |
| STORY-00.2.3 | `src/etcion/validation/` sub-package with `permissions.py` and `rules.py`; imports metamodel, never imported by metamodel |
| STORY-00.2.4 | `src/etcion/derivation/` sub-package with `engine.py`; leaf node in the dependency graph |
| STORY-00.2.5 | `src/etcion/exceptions.py` at the package root defining `PyArchiError`, `ValidationError`, `DerivationError`, `ConformanceError` |
