...
class DerivationEngine:
    ...
    def derive(self, model: Model) -> list[Relationship]:
        ...
        new :class:`~etcion.metamodel.concepts.Relationship` instances with
        ``is_derived=True``.

        Does NOT modify the source model.

        :param model: The model whose relationships to analyse.
        :returns: A list of new derived :class:`~etcion.metamodel.concepts.Relationship`
    ...
    def is_directly_permitted(
        ...
        Delegates to :func:`~etcion.validation.permissions.is_permitted`.
